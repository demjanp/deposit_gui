...
def svg_to_raster(path_svg, path_raster, 
	...
	for i, j, coords, transform, x0, y0 in idxs:
		...
		outline_path.setBrush(QtCore.Qt.GlobalColor.white)
		mask = QtGui.QPixmap(w, h)
		mask.fill(QtCore.Qt.GlobalColor.black)
		painter = QtGui.QPainter(mask)
		outline_path.paint(painter, QtWidgets.QStyleOptionGraphicsItem())
		painter.end()
		mask = mask.createMaskFromColor(QtCore.Qt.GlobalColor.white, QtCore.Qt.MaskMode.MaskOutColor)
		pixmap.setMask(mask)
	...
	image = QtGui.QImage(QtCore.QSize(w, h), QtGui.QImage.Format.Format_RGB32)
	image.setDotsPerMeterX(dpi * 39.37)
	image.setDotsPerMeterY(dpi * 39.37)
	image.fill(QtGui.QColor(QtCore.Qt.GlobalColor.white).rgb())
